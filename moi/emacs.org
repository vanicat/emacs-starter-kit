...
#+begin_src emacs-lisp
  ...
  (defun add-sub-load-path (dir)
    (push (expand-file-name dir dotfiles-dir) load-path))
...
* Les customs
  :PROPERTIES:
  :ID:       58c99dca-b283-4687-93cc-d6951cc5346d
  :END:
#+begin_src comment
...
#+begin_src emacs-lisp
  ...
  (setq user-mail-address "remi.vanicat@laposte.net")
...
*** mo-git-blame
    ...
    :END:
#+begin_src emacs-lisp
  (add-sub-load-path "mo-git-blame")
...
** config rails
   ...
   :END:
#+begin_src emacs-lisp
  (add-sub-load-path "rhtml")
  (add-sub-load-path "rinari")
  ...
  (load (expand-file-name "nxml/autostart.el" (expand-file-name "src" dotfiles-dir)))
